...
class JointProjector(Module):
    """project concat([vision,text]) into lm space"""
    def __init__(self, d=768):
        super().__init__()
        self.d = d
        self.dim_proj = Linear(2*d, d)

    def forward(self, vis_embeds, inputs_embeds, attention_mask):
        """
        vis_embeds: [8,1,768]
        inputs_embeds: [8,20,768]
        """
        vis_embeds = vis_embeds.repeat(1, inputs_embeds.shape[1], 1)
        vis_embeds = torch.einsum("ijk,ij->ijk", vis_embeds, attention_mask)
        x = torch.cat((inputs_embeds, vis_embeds), dim=-1)
        x = self.dim_proj(x)
        return x
...
class BartEncoder_P(BartEncoder):
    """
    BartEncoder + ViT + PromptEmbedding
    """
    def __init__(self, config, embed_tokens=None):
        ...
        # project vision space into lm space
        if config.vis_proj == 2:
            self.visual_projector = Linear(vit_config.hidden_size, config.d_model)
        elif config.vis_proj == 3:
            self.joint_projector = JointProjector(config.d_model)
    ...
    def set_vit(self, vit_path, vit_config):
        self.visual_embedding = self.visual_embedding.from_pretrained(vit_path, config=vit_config)
        # project vision space into lm space
        if self.config.vis_proj == 2:
            self.visual_projector = Linear(vit_config.hidden_size, self.config.d_model)
        elif self.config.vis_proj == 3:
            self.joint_projector = JointProjector(self.config.d_model)
    ...
    def forward(
        ...
        """Vision"""
        # get vision embeds, is no input image then create BLACK inputs
        if vis_feas is not None:
            if self.config.vis_proj == 0:  # cls, [B, 1, n_dim]
                vis_embeds = self.visual_embedding(vis_feas)[1].unsqueeze(1)
            elif self.config.vis_proj == 1:  # seq, [B, n_seq, n_dim]
                vis_embeds = self.visual_embedding(vis_feas)[0]
            elif self.config.vis_proj == 2:  # vp, [B, 1, n_dim]
                vis_embeds = self.visual_projector(self.visual_embedding(vis_feas)[1].unsqueeze(1))
            elif self.config.vis_proj == 3:  # jp
                vis_embeds = self.visual_embedding(vis_feas)[1].unsqueeze(1)
            vis_embeds = self.layernorm_vis(vis_embeds + self.embed_positions(vis_embeds.size()[:-1]))
        else:
            vis_embeds = None

        if self.config.vis_proj == 3:
            if vis_embeds is not None:
                """Joint project """
                inputs_embeds = self.joint_projector(vis_embeds, inputs_embeds, attention_mask)
            else:
                raise ValueError("vis_feas cannot be None if you want to do joint projection.")
            ...
            if self.config.prompt_pos == 2:    # right
                attention_mask = torch.cat((attention_mask, prompt_mask), dim=1)
                prompt_mask = torch.cat((torch.zeros(inputs_embeds.size()[:-1], dtype=dtype, device=device),
                                         prompt_mask), dim=1)
                inputs_embeds = torch.cat((inputs_embeds, prompt_embeds), dim=1)
            elif self.config.prompt_pos == 1:  # mid
                attention_mask = torch.cat((attention_mask[:, :vis_size[1], :],
                                            prompt_mask,
                                            attention_mask[:, vis_size[1]:, :]), dim=1)
                prompt_mask = torch.cat((torch.zeros(vis_size, dtype=dtype, device=device),
                                         prompt_mask,
                                         torch.zeros((B, L), dtype=dtype, device=device)), dim=1)
                inputs_embeds = torch.cat((inputs_embeds[:, :vis_size[1], :],
                                           prompt_embeds,
                                           inputs_embeds[:, vis_size[1]:, :]), dim=1)
            else:   # left
...
class BartModel_P(BartModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # add V, prompt to encoder_attention_mask
        if self.config.vis_proj == 1:
            attention_mask = torch.cat((torch.ones((attention_mask.size()[0], 197),
                                                   dtype=attention_mask.dtype, device=attention_mask.device),
                                        attention_mask), dim=1)
        elif self.config.vis_proj == 0 or self.config.vis_proj == 2:
            attention_mask = torch.cat((torch.ones((attention_mask.size()[0], 1),
                                                   dtype=attention_mask.dtype, device=attention_mask.device),
                                        attention_mask), dim=1)
        prompt_attention_mask = torch.ones((attention_mask.size()[0], self.config.prompt_len),
                                           dtype=attention_mask.dtype, device=attention_mask.device)
        if self.config.prompt_pos == 2:
...
class BartForConditionalGeneration_P(BartForConditionalGeneration):
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            if self.config.use_bce_loss:
                lm_logits_b = lm_logits[:, 1, [9904, 3084]]
                masked_lm_loss = loss_fct(lm_logits_b, (labels[:, 1] == 3084).to(torch.int64))
            else:
    ...
    #     }
    #
    #     # if "vis_feas" in kwargs:
    #     #     output["vis_feas"] = kwargs['vis_feas']
    #
    #     return output
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs
        ...
        if model_kwargs.get("vis_feas", None) is not None:
            model_kwargs['vis_feas'] = model_kwargs['vis_feas'].index_select(0, expanded_return_idx)
